scenario_mc <- function (base_estimate, scenarios, model_function, ..., numberOfModelRuns = NA, 
    ...
    for (i in parameter_lines) {
      ...
      if (!is.na(scenarios[i, scens])) {
        if (scenarios$param[i] == "both") {
          estim$marginal[vari, "lower"] <- as.numeric(scenarios[i, 
                                                                scens])
          estim$marginal[vari, "upper"] <- as.numeric(scenarios[i, 
                                                                scens])
        ...
        if (scenarios$param[i] == "distribution") 
          if (!is.na(scenarios[i, scens])) 
            estim$marginal[vari, "distribution"] <- scenarios[i, 
                                                              scens]
      }
    }
    scen_results <- mcSimulation(estimate = estim, model_function = model_function, 
                                 ..., numberOfModelRuns = n_Runs, functionSyntax = functionSyntax)
