...
decisionSupport::plot_distributions(mcSimulation_object = plotting_simulations,
                                    ...
                                    method = 'smooth_simple_overlay',
                                    x_axis_name = expression(atop("Crop water gap (% of needed irrigation water)", paste("for the no e-flows scenario"))),
                                    y_axis_name = "Distribution density",
...
monthly_crop_water_gap_baseline <- plot_cashflow(mcSimulation_object = plotting_simulations, 
              ...
              y_axis_name = "a",
              x_axis_name = "Month", 
              facet_labels = "") + labs(y = expression(atop("Monthly crop water gap (%)", paste("for the no e-flows scenario"))))
...
downstream_river_flow_baseline <- plot_cashflow(mcSimulation_object = plotting_simulations, 
              ...
              y_axis_name = "a",
              x_axis_name = "Month", 
              facet_labels = "") + labs(y = expression(Downstream~river~flow~(m^3)~no~e-flows))
...
Here the distribution of the changes in the crop water gap through environmental e-flows.
...
decisionSupport::plot_distributions(mcSimulation_object = plotting_simulations,
                                    ...
                                    method = 'smooth_simple_overlay',
                                    x_axis_name = expression(atop("Crop water gap change", paste("through environmental e-flows (percentage points)"))),
                                    y_axis_name = "Distribution density",
                                    base_size = 13) + theme(legend.position = "none")
...
monthly_crop_water_gap_environmental <- plot_cashflow(mcSimulation_object = plotting_simulations, 
              cashflow_var_name = "Crop_water_gap_difference_2_vs_1",
              y_axis_name = "a",
              x_axis_name = "Month", 
              facet_labels = "") + labs(y = expression(atop("Crop water gap change (perc. points)", paste("by environmental e-flows"))))
...
downstream_river_flow_env_flow <- plot_cashflow(mcSimulation_object = plotting_simulations, 
              ...
              y_axis_name = "a",
              x_axis_name = "Month", 
              facet_labels = "") + labs(y = expression(atop(Downstream~river~flow~change, paste(by~environmental~e-flows~(m^3)))))
...
decisionSupport::plot_distributions(mcSimulation_object = plotting_simulations,
                                    ...
                                    labs(x = expression(atop("Crop water gap change", paste("through livelihoods e-flows (percentage points)"))))
...
downstream_river_flow_live_flow <- plot_cashflow(mcSimulation_object = plotting_simulations, 
              ...
              y_axis_name = "a",
              x_axis_name = "Month", 
              facet_labels = "") + labs(y = expression(atop(Downstream~river~flow~change, paste(by~livelihoods~e-flows~(m^3)))))
...
We use the `plsr.mcSimulation` function of the `decisionSupport` package to run Partial Least Squares regression on the model outputs. We use the Variable Importance in the Projection (VIP) scores to identify important variables.
...
More information on all these procedures is contained in the [decisionSupport manual](https://cran.r-project.org/web/packages/decisionSupport/decisionSupport.pdf), especially under `welfareDecisionAnalysis`. This document was generated using the `rmarkdown` [@R-rmarkdown] and `knitr` [@R-knitr] packages in the R programming language [@R-base].

### Estimate values
 
```{r estimate_table, echo=F}
limpopo_input_table <- read.csv("data/limpopo_input_table.csv")
kable(limpopo_input_table, caption = 'This table contains the estimate values used for the Monte Carlo analysis') 
```
