...
# plotting_simulations$y[,paste0("cropwater_need",1:12)] <- plotting_simulations$y[,paste0("cropwater_need",1:12)]*100
# 
...
#            label= expression(Monthly~crop~water~need~(m^3))) 
...
Fig_monthly_baseline_crop_water_gap <- 
  plot_cashflow(mcSimulation_object = plotting_simulations, 
...
monthly_crop_water_gap_suppl <- plot_cashflow(mcSimulation_object = plotting_simulations, 
...
Fig_monthly_baseline_crop_water_gap +
  ...
  #plot_annotation(title = "Monthly crop water gap for all e-flows scenarios")
...
Fig_eflow_policy_effect_crop_water_gap <- decisionSupport::plot_distributions(mcSimulation_object = plotting_simulations,
                    ...
                    ggplot2::theme_classic() + 
                    ggplot2::labs(fill = "")  +
                     ggplot2::scale_fill_discrete(labels=c('UNRES (unrestricted) baseline', 'EFLOW abstrction control', 'SUPPL dam releases')) 
...
Fig_monthly_downstream_river_flow <-
  plot_cashflow(mcSimulation_object = plotting_simulations, 
              ...
              x_axis_name = "", 
              facet_labels = "") + 
  annotate(geom="text", x=3, y= 200000000, 
...
Fig_monthly_change_in_downstream_river_flow_env_eflows <-
  plot_cashflow(mcSimulation_object = plotting_simulations, 
              ...
              x_axis_name = "", 
              facet_labels = "") +  
  annotate(geom="text", x=3, y= 250000, 
            label=(expression(atop("EFLOW", 
                                   paste("abstrction control"))))) + 
          theme( axis.text.x = element_blank(),
          axis.ticks = element_blank()) + 
...
#('UNRES (unrestricted) baseline', 'EFLOW abstrction control', 'SUPPL dam releases')) 

```

```{r plot_downstream_flow, include = F}
library(patchwork)
...
Fig_15_PLS_irrigation_water_need <- plot_pls(pls_result_irrig_water_need, 
                            input_table = input_table, 
                            threshold = 1, 
                            x_axis_name = expression(atop("Variable Importance in the Projection", 
                                                          paste("score for annual irrigation water needs"))))
...
PLS_UNRES_baseline_crop_water_gap <- plot_pls(pls_result_1, 
                            input_table = input_table, 
                            threshold = 1, 
                            x_axis_name = expression(atop("Variable Importance in the Projection", paste("score for crop water gap for UNRES – baseline, unrestricted water use scenario"))),
                            y_axis_name = "Model input variables")
...
ggsave("figures/Fig_7_sensitivity.png", width=7, height=10)
