...
* Scenario 2 - restricted extraction: This is an eflow scenario, in which eflows are interpreted in a purely ecological sense. Whenever eflows aren't achieved, water extraction is curtailed. There are no measures to add water to the river in such events. We simulate the baseline with our own functions and some from the `nasapower` [@R-nasapower] and `Evapotranspiration` [@R-Evapotranspiration] packages.
...
return(list(scen1_downstream_river_flow=mean(scen1_river_flow_downstream),
...
Here we plot the difference in farm level crop water gap (i.e. how much water is missing from the smallholder farmers needs) between the baseline Scenario 1, no eflows and eflow Scenario 2, restricted extraction. We simulated this scenario so that whenever eflows aren't achieved, water extraction is curtailed and there are no measures to add water to the river.

```{r}
gap_2_n_1_plot <- decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results,
                                    vars = c("scen2_crop_water_gap", "scen1_crop_water_gap"),
                                    method = 'smooth_simple_overlay',
                                    colors = c("#D55E00", "#CC79A7"),
                                    x_axis_name = "Scenario 2 and Baseline",
                                    base_size = 7)

gap_2_vs_1_plot <- decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results,
                                    vars = c("Mean_Crop_water_gap_difference_2_vs_1"),
                                    method = 'smooth_simple_overlay',
                                    x_axis_name = "Scenario 2 vs. Baseline",
                                    y_axis_name = "",
                                    base_size = 7)
...
gap_2_n_1_plot + gap_2_vs_1_plot + 
...
Here we plot the difference in farm level crop water gap (i.e. how much water is missing from the smallholder farmers needs) between the baseline Scenario 1, no eflows and Scenario 3, dam releases. We simulate this scenario so that when eflows aren't naturally met, water is released from upstream dams to ensure eflows. Smallholder farmers can access water so long as their pumps are working.
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results,
                                    ...
                                    method = 'smooth_simple_overlay',
                                    x_axis_name = "Scenario 3 vs. Baseline",
                                    base_size = 7)
...
These are the flows for scenario 2 and for scenario 3 vs. the no-eflow baseline:
...
pls_plot_2_vs_1 <- plot_pls(pls_result_2_vs_1, 
                            input_table = input_table, 
                            threshold = 0.5, 
                            x_axis_name = "",
                            y_axis_name = "Scenario 2 vs. Baseline")
...
pls_plot_3_vs_1 <- plot_pls(pls_result_3_vs_1, 
                            input_table = input_table, 
                            threshold = 0.5, 
                            y_axis_name = "Scenario 3 vs. Baseline")
...
pls_plot_2_vs_1 + pls_plot_3_vs_1 + 
plot_layout(guides = "collect", design = layout) & 
  theme(legend.position='bottom')

```

The VIP scores show us the importance of each variable in the projection used in our PLS models for the mean crop water gap in scenario 2 and 3 vs. the baseline. 
